...
| metadata | object | An opaque object of metadata for a service offering. Controller treats this as a blob. Note that there are [conventions](profile.md#service-metadata) in existing brokers and controllers for fields that aid in the display of catalog data. |
...
| metadata | object | An opaque object of metadata for a service plan. Controller treats this as a blob. Note that there are [conventions](profile.md#service-metadata) in existing brokers and controllers for fields that aid in the display of catalog data. |
...
| --- | --- | --- |
| service_id* | string | MUST be the ID of a service from the catalog for this broker. |
...
| parameters | object | Configuration options for the service instance. Brokers SHOULD ensure that the client has provided valid configuration parameters and values for the operation. |
...
| service_id* | string | MUST be the ID of a service from the catalog for this broker. |
| plan_id | string | If present, MUST be the ID of a plan from the service that has been requested. If this field is not present in the request message, then the broker MUST NOT change the plan of the instance as a result of this request. |
...
| parameters | object | Configuration options for the service binding. Brokers SHOULD ensure that the client has provided valid configuration parameters and values for the operation. |
...
`:binding_id` MUST be the the ID of a previously provisioned binding for
